def pad_image(image, char='.', padding=3):
    img_y = len(image)
    img_x = len(image[0])
    end_x = img_x + (2 * padding)

    for i in range(img_y):
        line = ''.join([char for _ in range(padding)]) + \
            image[i] + ''.join([char for _ in range(padding)])
        image[i] = line

    for i in range(padding):
        line = ''.join([char for _ in range(end_x)])
...
for _ in range(0, 50):
    ...
    image = pad_image(image, image[0][0], 2)

print(bright_count(image))
